...
// Category hero images for visual cards
const CATEGORY_IMAGES: Record<string, string> = {
  "ai-writing": "/images/cat-writing.webp",
  "ai-coding":  "/images/cat-coding.webp",
  "ai-image":   "/images/cat-image-gen.webp",
};
...
export function CategoryCard({ category }: { category: Category }) {
  ...
  return (
    <Link
      ...
      className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br ${colors.gradient} transition-all duration-300 hover:border-brand-400/30 hover:shadow-lg hover:shadow-brand-900/20 hover:-translate-y-0.5`}
    >
      {/* Hero image for featured categories */}
      {heroImage ? (
        <>
          <div className="relative h-36 w-full overflow-hidden">
            ...
            <div className={`absolute bottom-3 left-4 flex h-10 w-10 items-center justify-center rounded-xl bg-black/40 text-xl backdrop-blur-sm ring-1 ${colors.ring}`}>
              {category.icon}
            </div>
          </div>
          <div className="p-5">
            <h3 className="mb-1.5 text-base font-bold text-white transition-colors group-hover:text-brand-300">
              {category.name}
            </h3>
            <p className="mb-4 text-sm leading-relaxed text-gray-500">{category.description}</p>
